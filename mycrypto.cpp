    ...
    amount_of_bytes(size_)
{
    if(!size_) pBlocks = nullptr;
        ...
        std::unique_ptr newblock = std::make_unique<BYTE>(size_);
        pBlocks = newblock.get();
        memset(pBlocks, init_value, size_);
    }
}
ByteBlock::ByteBlock(const BYTE * pBlocks_, size_t size_) :
    amount_of_bytes(size_)
{
    std::unique_ptr newblock = std::make_unique<BYTE>(size_);
    pBlocks = newblock.get();
...
/*
Arguments:
	const ByteBlock by reference
Return:
    string
Description:
	Преобразует блок байт в hex string.
*/
...
Return:
    ByteBlock
Description:
	Создаёт объект ByteBlock из hex string.
*/
ByteBlock hex_to_bytes(const string & s)
{
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        std::cout << "t: " << t << "result[i]" << result[i] << std::endl;
